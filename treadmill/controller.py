...
class TreadmillController:
    ...
    async def _write_command(self, command):
        try:
            response = await self.client.write_gatt_char(self.control_point_uuid, command, response=True)
            print(f"Written command '{command.hex}' to treadmill. Response: '{response}'")
        except Exception:
...
if __name__ == "__main__":
    ...
    async def main():
        async with BleakClient(treadmill_address) as client:
            controller = TreadmillController(client, control_point_uuid, data_point_uuid)
